# Introduction to Web Design

<div style="font-size:1.3em">
    <p>We have heard of the saying:</p>
    <p>
        <i>"Don't judge a book by its cover."</i>
    </p>
    <p>At the same time we have heard of these saying:</p>
    <p>
        <i>"You only have 3 seconds to make a good impression."</i>
    </p>
    <p>And when it comes to Web Design, the second is more accurate.</p>
    <p>Let's look at the same site but with a different design</p>
    <ul>
        <li>
            <a href="https://www.pennyjuice.com/">
                https://www.pennyjuice.com/
            </a>
            <div style="text-align: center;">
                <img src="assets/1.png" alt="" style="width: 540px;">
            </div>
        </li>
        <li>
            <a href="https://www.behance.net/gallery/40393701/Penny-Juice-Rebrand">
                https://www.behance.net/gallery/40393701/Penny-Juice-Rebrand
            </a>
            <div style="text-align: center;">
                <img src="assets/2.png" alt="" style="height: 240px;">
                <img src="assets/3.png" alt="" style="height: 240px;">
                <img src="assets/4.png" alt="" style="height: 240px;">
                <img src="assets/5.png" alt="" style="height: 240px;">
                <img src="assets/6.png" alt="" style="height: 240px;">
            </div>
        </li>
    </ul>
    <p>
        Ask yourself: If you were to buy this product from the company—if you were to purchase a carton of this 
        juice—how much would you pay for it?
    </p>
    <h3>Understanding Color Theory</h3>
    <div style="text-align: center;">
        <img src="assets/7.png" alt="" style="width: 480px;">
    </div>
    <p>
        Color Palettes for Designers and Artists:
        <a href="https://colorhunt.co/">https://colorhunt.co/</a>
    </p>
    <div style="text-align: center;">
        <img src="assets/8.png" alt="" style="height: 240px;">
        <img src="assets/9.png" alt="" style="height: 240px;">
    </div>
    <h3>Topography</h3>
    <h2>Understanding Topography and How to Choose Fonts</h2>
    <ul>
        <li>
            Serif Family
            <ul>
                <li>Old Style</li>
                <li>Transitional</li>
                <li>Modern</li>
                <li>Slab-Serif</li>
            </ul>
        </li>
        <li>
            Sans-Serif (More Readable)
            <ul>
                <li>Gill Sans is a Humanist sans-serif</li>
                <li>News Gothic is a Grotesque sans-serif typeface</li>
            </ul>
        </li>
    </ul>
    <p>Fonts also have moods:</p>
    <div style="text-align: center;">
        <img src="assets/10.png" alt="" style="width: 620px;">
    </div>  
    <p>Readability and Legibility are important when choosing fonts.</p>
    <p>Recommendation: to keep it cleaner and tighter - stick to two fonts.</p>
    <h3>User Interface Design (UI)</h3>
    <div style="text-align: center;">
        <img src="assets/11.png" alt="" style="width: 540px;">
    </div>    
    <div>
        <ol>
            <li>
                Hierarchy
                <ul>
                    <li>Color</li>
                    <li>Size</li>
                </ul>
            </li>
            <li>Layout</li>
            <li>Alignment</li>
            <li>White Space</li>
            <li>Audience</li>
        </ol>
    </div>
    <h3>User Experience Design (UX)</h3>
    <div>
        <ol>
            <li>Simplicity</li>
            <li>Consistency</li>
            <li>
                Reading Patterns
                <ul>
                    <li>F Layout</li>
                    <li>Z Layout</li>
                </ul>
            </li>
            <li>All Platform Design</li>
            <li>Don't Use Your Powers for Evil</li>
        </ol>
    </div>
    <div>
        <h4>Web Design in Practice</h4>
        <ul>
            <li><a href="https://www.canva.com/">canva.com</a></li>
            <li><a href="hotel_design.pdf">my hotel design</a></li>
        </ul>
    </div>
</div>
